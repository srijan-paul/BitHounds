...
class Hound(sp.Contract):
    ...
    def createHound(self, params):
        ...
        sp.set_type(params.ipfs, sp.TBytes)
        ...
        sp.transfer(sp.record(creator = sp.sender, token_id = self.data.counter, metadata = sp.map(l = {"" : params.ipfs})), sp.mutez(0), token_contract)
    ...
    def buy(self, params):
        ...
        token_contract = sp.contract(sp.TRecord(from_ = sp.TAddress, to_ = sp.TAddress, token_id = sp.TNat, metadata = sp.TMap(sp.TString, sp.TBytes)), self.data.contract_address, entry_point = "transfer").open_some()
        sp.transfer(sp.record(from_ = hound.owner, to_ = sp.sender, token_id = params.hound_id, metadata = sp.map(l = {"" : params.ipfs})), sp.mutez(0), token_contract)
...
def test():
    ...
    c1.createHound(genome = "8Bxh1qpQn2Xz6Ip0cAyzAbfLCdlUlPFw4Qzvjk2I", generation = 0, timestamp = 1324, ipfs = sp.pack("abcd")).run(sender = mark)
    c1.createHound(genome = "otQMyicGthNPFnv52tmf8wUW7yRLqxTFFQ4Eisji", generation = 1, timestamp = 1532, ipfs = sp.pack("efgh")).run(sender = mark)
    c1.sell(hound_id = 1, price = sp.mutez(21)).run(sender = mark)
    c1.buy(hound_id = 1, ipfs = sp.pack("ijkl")).run(sender = bill, amount=sp.mutez(21))