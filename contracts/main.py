...
class FA2_token_metadata(FA2_core):
    ...
    def make_metadata_ipfs(symbol, name, decimals, ipfs):
        "Helper function to build metadata JSON bytes values."
        return (sp.map(l = {
            # Remember that michelson wants map already in ordered
            "decimals" : sp.utils.bytes_of_string("%d" % decimals),
...
class Hound(sp.Contract):
    ...
    def createHound(self, params):
        ...
        sp.transfer(sp.record(creator = sp.sender, token_id = self.data.counter, metadata = FA2_token_metadata.make_metadata_ipfs(
            decimals = 0,
            name = 'Hound NFT',
            symbol = 'Hound',
            ipfs = params.ipfs
...
def test():
    ...
    scenario  = sp.test_scenario()
    scenario.h1("Hounds")
    nft = NFT(FA2_config(non_fungible = True), admin = admin, metadata = sp.big_map({"": sp.utils.bytes_of_string("tezos-storage:content"),"content": sp.utils.bytes_of_string("""{"name" : "Hound", "author": "Test", "status": "Dev"}""")}))
    scenario += nft
    c1 = Hound(nft.address)
    scenario += c1  

    c1.createHound(genome = "8Bxh1qpQn2Xz6Ip0cAyzAbfLCdlUlPFw4Qzvjk2I", generation = 0, timestamp = 1324, ipfs="abcd").run(sender = mark)
    c1.createHound(genome = "otQMyicGthNPFnv52tmf8wUW7yRLqxTFFQ4Eisji", generation = 1, timestamp = 1532, ipfs="efgh").run(sender = mark)
